...
The method is also distinct from recent work improving the asymptotic exponent $\omega$ using large-scale continuous optimization and AlphaEvolve \cite{dupont2026}: our objective here is the exact finite tensor-rank landscape of small matrix multiplication, not the laser-method optimization underlying asymptotic bounds. The post-discovery complexity experiment additionally suggests that the same exact family can be navigated according to an objective other than tensor rank.
...
Indeed, additive complexity is a separate optimization problem.  Recent rank-23 work has reduced the linear-circuit cost to 55 additions/subtractions, for 78 scalar operations including the 23 bilinear products \cite{karunaratne2026}.  The frozen rank-discovery objective did not penalize coefficient alphabet, sparsity of linear forms, constant multiplications, or addition count.  This separation is experimentally useful: the repeated discovery of rank 23 is not a consequence of constraining the search toward known sparse ternary schemes.

\subsection{Fixed-rank complexity navigation}

After the frozen discovery experiment, we tested whether a discovered exact rank-23 family could be navigated according to raw linear-form support.  Tensor equality remained a hard constraint.  A smooth support objective was projected into the local exact tangent space, candidate steps were corrected back to the rank-23 solution set, existing structural zeros were preserved, and newly tiny coefficients were tested as exact zero constraints.

The first campaign started from the exact seed-211 and seed-401 representatives.  Table~\ref{tab:complexity} reports the raw support-derived addition count before the campaign, at the best numerical sparse checkpoint, and after exactification.  A $k$-term linear form is charged $k-1$ additions/subtractions.

\begin{table}[ht]
\centering
\caption{Post-discovery complexity navigation.  Greedy CSE is a deterministic exact straight-line-program upper bound; arbitrary scalar constant multiplications are not charged.}
\label{tab:complexity}
\begin{tabular}{rccccc}
\toprule
Seed & Start naive & Numerical & Exact naive & Greedy CSE & Exact field \\
\midrule
211 & 143 & 127 & 124 & $94\to82$ & rational \\
401 & 137 & 128 & 109 & $87\to71$ & $\QQ(\sqrt{23})$ \\
\bottomrule
\end{tabular}
\end{table}

All deliberately frozen campaign zeros remain exact zeros in the resulting symbolic certificates, and all 729 Brent identities verify exactly.  For seed 401 the quadratic generator may be chosen to satisfy $23\alpha^2-4=0$.  The substantial additional simplification during exactification, particularly $128\to109$ for seed 401, shows that the numerical search can locate a low-complexity neighbourhood whose exact local family contains an even simpler representative.

These counts are not claims of optimal additive complexity.  The greedy CSE pass is heuristic, and arbitrary scalar multiplications are free in this accounting.  In particular, the value 71 is not directly comparable with a ternary 55-addition construction on total scalar-operation cost.

\subsection{Does simplicity help rank discovery?}

We also ran a matched ablation in which the local rank-reduction operators, shared rank-26 starting checkpoint, beam width, and expansion budget were unchanged, while a weak complexity score influenced beam policy.  Baseline, constant-weak, and rank-adaptive policies all reached rank 23 on all five frozen seeds and all had median structural endpoint complexity 566 additions.  Four seeds reproduced the baseline generation count.  Seed 211 was the exception: baseline required 12 beam generations, constant weak guidance required 29 with no endpoint-complexity benefit, and a policy delayed until rank 24 returned to 12 generations.  Delayed runs on seeds 307 and 503 also matched baseline.

At the tested strength, arithmetic simplicity therefore supplies no demonstrated benefit during rank discovery and can perturb a productive earlier basin trajectory.  In contrast, it is highly effective as a post-discovery objective on a fixed rank-23 family.  The empirical architecture is consequently staged: discover rank first, then simplify the exact family.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item The reported greedy-CSE addition counts are upper bounds under an addition-only cost convention that leaves arbitrary scalar constant multiplications uncharged.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \texttt{run\_rank23\_complexity\_campaign.py}: fixed-rank support-reduction campaign;
    \item \texttt{run\_rank23\_complexity\_followup.py}: exactification and exact circuit comparison of sparse winners;
    \item \texttt{run\_complexity\_guidance\_ablation.py}: matched complexity-during-discovery ablation.
\end{enumerate}
...
The complexity experiments suggest that different stages benefit from different objectives: simplicity did not help the frozen rank-reduction search at the tested strength, but became effective once rank 23 was fixed.  This motivates a modular view of algorithm search rather than a single scalar objective.

Three follow-up experiments are especially informative.  First, compare against deliberately simple baselines: plain gradient descent or Adam from the same schoolbook start, plus a simple sparsity/death-regularized variant, under matched seeds and approximate compute budgets.  Second, extend the post-discovery cost model to coefficient alphabet, nontrivial scalar-multiplication cost, and stronger straight-line-program optimization.  Third, apply the same method to $4\times4$ multiplication, initially asking only whether it can reproduce a known rank reduction before attempting unexplored boundaries.  A rank-22 campaign remains a high-upside exploratory direction; any success would require finite coefficients, high-precision refinement, and exact verification, while failure to find a crossing is not a lower bound.
...
A differential-geometric, basin-search view of bilinear algorithms can repeatedly navigate from schoolbook $3\times3$ multiplication to the known rank-23 boundary without being supplied a target rank-23 decomposition during evaluation.  In a frozen five-seed experiment, all five runs reached rank 23, and several resulting endpoints admit independently verifiable exact symbolic certificates.  The current evidence does not establish a new rank record or new continuous families.  It does establish a reproducible computational pathway from an elementary exact algorithm, through a sequence of exact solution basins, to structurally diverse low-rank algorithms. The post-discovery experiments further show that a discovered exact rank-23 family can be navigated toward substantially lower-support exact representatives, whereas the same simplicity signal provides no benefit during the frozen rank-reduction phase at the tested strength. Whether the machinery can cross the rank-23 boundary or generalize to other bilinear tensors remains open.
