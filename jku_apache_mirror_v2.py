...
def main() -> int:
    ...
    with cf.ThreadPoolExecutor(max_workers=max(1, args.workers)) as ex:
        while (pending_dirs or pending_files or active) and not stop.is_set():
            ...
            while len(active) < args.workers and not stop.is_set():
                if pending_dirs:
                    ...
                    u = pending_files.popleft()
                    fut = ex.submit(process_file, u)
                    active[fut] = ("file", u)
            ...
            if completed and completed % 250 < len(done):
                print(
                    ...
                    f"queued_dirs={len(pending_dirs)} queued_files={len(pending_files)} active={len(active)} "
    ...
    summary = {
        ...
        **dict(stats),
        ...
        "queued_dirs_remaining": len(pending_dirs),
        "queued_files_remaining": len(pending_files),
        "queue_remaining": len(pending_dirs) + len(pending_files),
    }
